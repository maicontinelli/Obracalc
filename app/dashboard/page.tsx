...
export default function DashboardPage() {
    ...
    const [activeSection, setActiveSection] = useState<'overview' | 'orcamentos' | 'perfil' | 'assinatura'>('overview');
    ...
    const firstName = profileData.full_name?.split(' ')[0] || 'Parceiro';

    // Plan badge
    const PlanBadge = () => {
        if (subscriptionInfo.tier === 'business') {
            return (
                <span className="inline-flex items-center gap-1 text-[10px] font-bold px-2.5 py-1 rounded-full bg-teal-50 dark:bg-teal-900/30 text-teal-700 dark:text-teal-400 border border-teal-200 dark:border-teal-800">
                    ✦ Empresarial
                </span>
            );
        }
        if (subscriptionInfo.tier === 'pro') {
            return (
                <span className="inline-flex items-center gap-1 text-[10px] font-bold px-2.5 py-1 rounded-full bg-sky-50 dark:bg-sky-900/30 text-sky-700 dark:text-sky-400 border border-sky-200 dark:border-sky-800">
                    ✦ Profissional
                </span>
            );
        }
        return (
            <span className="inline-flex items-center gap-1 text-[10px] font-bold px-2.5 py-1 rounded-full bg-gray-100 dark:bg-white/5 text-gray-500 dark:text-gray-400 border border-gray-200 dark:border-white/10">
                Grátis
            </span>
        );
    };
    ...
    const navItems = [
        ...
        { id: 'perfil' as const, label: 'Meu Perfil', icon: UserIcon },
        { id: 'assinatura' as const, label: 'Assinatura', icon: CreditCard },
    ];

    const ferramentasItems = [
        { label: 'Relatório de Obra', href: '/relatorio-fotografico', icon: Camera, color: 'text-indigo-500' },
        { label: 'Orçamento por Imagem', href: '/novo-diagnostico', icon: Sparkles, color: 'text-teal-500' },
        { label: 'Topografia', href: '/topografia', icon: Map, color: 'text-orange-500' },
    ];
        ...
        <div className="flex flex-col h-full">
            ...
            <div className="px-4 py-4 border-b border-black/5 dark:border-white/5">
                <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-full overflow-hidden bg-gray-200 dark:bg-gray-700 shrink-0 flex items-center justify-center">
                    ...
                    <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold text-foreground truncate">{profileData.full_name || 'Meu Perfil'}</p>
            ...
            <nav className="flex-1 px-2 py-4 space-y-0.5">
                ...
                <div className="pt-4 pb-1 px-3">
                    <span className="text-[9px] font-bold uppercase tracking-widest text-muted-foreground/60">Ferramentas</span>
                </div>
                ...
                {ferramentasItems.map(item => {
                    ...
                    return (
                        <button
                            key={item.href}
                            onClick={() => {
                                handleFeatureAction(item.href);
                                setIsSidebarOpen(false);
                            }}
                            className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-white/5 transition-colors"
                        >
            ...
            <div className="p-4 border-t border-black/5 dark:border-white/5 space-y-3">
                <button
                    onClick={() => { handleNewBudget(); setIsSidebarOpen(false); }}
                    className="w-full bg-teal-600 hover:bg-teal-700 text-white text-sm font-bold py-2.5 rounded-xl transition-colors flex items-center justify-center gap-2"
                >
                    <Plus size={16} />
                    Novo Orçamento
                </button>
                <button
                    ...
                    className="w-full flex items-center gap-2 px-2 py-1.5 text-xs text-gray-400 hover:text-red-500 transition-colors rounded-lg"
                >
                    <LogOut size={14} />
                    Sair da conta
                </button>
    ...
    const cardClass = 'bg-white/70 dark:bg-[#1A1A1A]/70 backdrop-blur-xl rounded-2xl border border-black/5 dark:border-white/5 shadow-sm';
    ...
    return (
        <div className="min-h-screen bg-gray-50 dark:bg-background flex">
            ...
            {/* Sidebar panel */}
            <aside
                className={`fixed inset-y-0 left-0 z-50 w-60 bg-white dark:bg-[#141312] border-r border-black/5 dark:border-white/5 pt-20 flex flex-col transition-transform duration-200
            ...
            {/* ── MAIN CONTENT ── */}
            <main className="ml-0 lg:ml-60 flex-1 p-6 lg:p-8 min-w-0">

                {/* Banners */}
                <div className="space-y-3 mb-6">
                    {subscriptionInfo.status === 'past_due' && (
                        <div className="flex items-start gap-4 bg-red-50 dark:bg-red-950/40 border border-red-300 dark:border-red-800 rounded-xl p-4 text-sm">
                    ...
                    {subscriptionInfo.status === 'active' && subscriptionInfo.paymentFailures === 1 && (
                        <div className="flex items-start gap-4 bg-teal-50 dark:bg-teal-950/40 border border-teal-200 dark:border-teal-800 rounded-xl p-4 text-sm">
                ...
                {activeSection === 'overview' && (
                    <div className="space-y-6">
                        <div>
                            <h1 className="text-2xl font-semibold text-foreground tracking-tight">Olá, {firstName}</h1>
                            <p className="text-sm text-muted-foreground mt-0.5">Seus números de hoje</p>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div className={`${cardClass} p-6 flex flex-col justify-center`}>
                                ...
                                <p className="text-3xl font-bold mt-auto pb-2">
                            ...
                            {/* Tempo */}
                            <div className={`${cardClass} p-6 flex flex-col relative overflow-hidden group`}>
                        ...
                        <div className={`${cardClass} p-6`}>
                            <div className="flex justify-between items-center mb-4">
                                <h3 className="text-sm font-semibold text-foreground">Orçamentos Recentes</h3>
                                <button
                        ...
                        <div className={`${cardClass} p-6 space-y-6`}>
                            ...
                            <div className="flex justify-end pt-2">
                                <button
                                    ...
                                    className={`px-8 py-3 text-sm font-bold rounded-xl shadow-lg transition-all active:scale-95 text-white flex items-center gap-2 ${saveSuccess ? 'bg-green-500 shadow-green-500/20' : 'bg-primary hover:bg-primary/90 shadow-primary/20'}`}
                                >
                ...
                {activeSection === 'assinatura' && (
                    <div className="space-y-6">
                        <h1 className="text-2xl font-semibold text-foreground tracking-tight">Assinatura</h1>
                        ...
                        {subscriptionInfo.tier !== 'free' ? (
                            <div className={`${cardClass} p-6 space-y-4`}>
                                <div className="flex justify-between items-center pb-2 border-b border-border/50">
                                    ...
                                    {subscriptionInfo.status === 'past_due' && (
                                        <span className="text-[10px] text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700 rounded-lg px-2 py-1">
                                ...
                                <div className="space-y-3">
                                    <div className="flex justify-between items-end">
                                        <span className="text-4xl font-bold">{subscriptionInfo.daysRemaining ?? '∞'}</span>
                                ...
                                <div className="pt-3 border-t border-border/30 flex flex-col gap-2">
                                    <button
                                        ...
                                        className="w-full text-sm font-bold flex items-center justify-center gap-2 py-2.5 bg-muted/50 hover:bg-muted rounded-xl transition-colors"
                                        title="Baixar Recibo de Pagamento"
                                    >
                                        <Download size={14} /> {isDownloadingReceipt ? 'Gerando...' : 'Baixar Recibo'}
                                    </button>
                                    {subscriptionInfo.inGuaranteeWindow && (
                                        <button
                                            onClick={handleCancelSubscription}
                                            disabled={isCanceling}
                                            className="w-full text-sm text-red-500 hover:text-red-600 font-bold py-2 flex items-center justify-center gap-2"
                                        >
                                            {isCanceling ? <Loader2 className="animate-spin" size={14} /> : null}
                                            Cancelar Assinatura
                                        </button>
                                    )}
                                </div>
                            ...
                            <div className={`${cardClass} p-8 text-center space-y-4`}>
                                <div className="w-12 h-12 rounded-2xl bg-teal-50 dark:bg-teal-900/20 flex items-center justify-center mx-auto">
                                    <CreditCard size={24} className="text-teal-600 dark:text-teal-400" />
                                </div>
                                <div>
                                    <h3 className="font-bold text-lg text-foreground">Você está no plano gratuito</h3>
                                    <p className="text-sm text-muted-foreground mt-1">Faça upgrade para desbloquear todas as ferramentas e recursos premium.</p>
                                </div>
                                <button
                                    onClick={() => router.push('/planos')}
                                    className="bg-teal-600 hover:bg-teal-700 text-white font-bold px-8 py-3 rounded-xl transition-colors"
                                >
                                    Ver Planos
                                </button>
                            </div>
            ...
            <button
                className="lg:hidden fixed bottom-6 right-6 z-50 w-12 h-12 bg-teal-600 hover:bg-teal-700 text-white rounded-full shadow-lg flex items-center justify-center transition-colors"
